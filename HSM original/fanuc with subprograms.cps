...
description = "Generic FANUC with subprograms";
vendor = "Fanuc";
...
longDescription = "Generic FANUC post which has built-in support for subprograms. By default each operation will be output in separate files. If you turn off the 'useFilesForSubprograms' property you will get the subprograms embedded in the main program.";
...
function onSection() {
  ...
  var makeSubprogram = true;
  if (false) {
    var subprogramLevel = 3;
    makeSubprogram = false;
    if (hasParameter("hsmworks:path")) { // autodeskcam
      var path = getParameter("hsmworks:path"); // autodeskcam
      var sections = path.split("\\");
      var level = sections.length;
      makeSubprogram = level >= subprogramLevel;
    }
  }
  
  if (makeSubprogram) { // when to do a subprogram
    var programId;
    try {
      programId = getAsInt(programName);
    } catch(e) {
