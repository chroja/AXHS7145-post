/**
  ...
  $Revision: 27151 $
  $Date: 2011-10-13 17:44:15 +0200 (to, 13 okt 2011) $
...
var pFormat = createFormat({prefix:"P", width:2, zeropad:true, decimals:1});
...
function onOpen() {
  ...
  if (programName) {
    ...
    if ((programId >= 7000) && (programId <= 9999)) {
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    writeBlock(gFormat.format(53), "Z" + xyzFormat.format(0)); // retract
    writeBlock(gFormat.format(53), "X" + xyzFormat.format(-570), "Y" + xyzFormat.format(0)); // retract
    // writeBlock("H" + toolFormat.format(tool.number));
...
function onCyclePoint(x, y, z) {
  if (isFirstCyclePoint()) {
    ...
    case "boring":
      ...
      writeBlock(mFormat.format(3)); // start spindle
        writeBlock(
          //gRetractModal.format(98), gAbsIncModal.format(90), gCycleModal.format(85),
          //getCommonCycle(x, y, cycle.bottom, cycle.retract),
          //feedOutput.format(F)
          gAbsIncModal.format(90), gCycleModal.format(1),
          getCommonCycle(x, y, cycle.bottom, cycle.retract),
          feedOutput.format(F)
        );
        writeBlock(mFormat.format(5)); // stop spindle
...
function onRapid5D(_x, _y, _z, _a, _b, _c) {
  ...
  if (currentSection.isOptimizedForMachine()) {
    ...
    writeBlock(gMotionModal.format(0), gFormat.format(43.5), x, y, z, "I" + i, "J" + j, "K" + k);
  }
  feedOutput.reset();
}
...
function onLinear5D(_x, _y, _z, _a, _b, _c, feed) {
  ...
  if (currentSection.isOptimizedForMachine()) {
    ...
    var f = feedOutput.format(feed);
    if (x || y || z || i || j || k) {
      writeBlock(gMotionModal.format(1), gFormat.format(43.5), x, y, z, "I" + i, "J" + j, "K" + k, f);
    } else if (f) {
      if (getNextRecord().isMotion()) { // try not to output feed without motion
        feedOutput.reset(); // force feed on next line
      } else {
...
function onCommand(command) {
  ...
  case COMMAND_CALIBRATE:
	...
	writeBlock("/", gFormat.format(65), pFormat.format(9023), mFormat.format(1));
	writeBlock("/", mFormat.format(1));
	writeBlock("(BLOK PO BLOKU!)");
	writeBlock("(MERENI V OSE X)");
	writeBlock("/", gFormat.format(65), pFormat.format(9770),"X" + xyzFormat.format(0), "Y" + xyzFormat.format(0), feedOutput.format(10001), "(zadej souradnice x y)");
	writeBlock("/", gFormat.format(65), pFormat.format(9770),"Z" + xyzFormat.format(50), feedOutput.format(8000), "(zadej souradnice z)");
	writeBlock("/", gFormat.format(65), pFormat.format(9770),"Z" + xyzFormat.format(-5), feedOutput.format(3000), "(zadej souradnice z)"); 
	writeBlock("/", gFormat.format(65), pFormat.format(9023),"X" + xyzFormat.format(0), "S" + xyzFormat.format(54), mFormat.format(2), "(zadej rozsah mereni)");
	writeBlock("/", gFormat.format(65), pFormat.format(9770),"Z" + xyzFormat.format(50), feedOutput.format(3001));
	writeBlock("/", gFormat.format(65), pFormat.format(9023), mFormat.format(3)); 
	writeBlock("/", gFormat.format(53), "Z" + xyzFormat.format(10)); // retract
  	// writeBlock("N100");
  ...
  case COMMAND_STOP://0,
    ...
    writeBlock(mFormat.format(5)); // stop spindle
    writeBlock(mFormat.format(9)); // stop coolant
    writeBlock(mFormat.format(15)); // stop coolant
