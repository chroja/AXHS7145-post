...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    retracted = true;
    writeBlock(mFormat.format(26)); // retract
  ...
  if (workOffset >= 0) {
    if (workOffset > 9) {
      ...
      writeBlock(hFormat.format(workOffset)); // H0-H9
    }
  }
...
function onClose() {
  ...
  if (!machineConfiguration.hasHomePositionX() && !machineConfiguration.hasHomePositionY()) {
    writeBlock(gAbsIncModal.format(90), gFormat.format(0), "X" + xyzFormat.format(0), "Y" + xyzFormat.format(0)); // return to home
  } else {
    var homeX;
