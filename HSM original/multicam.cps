...
properties = {
  ...
  showSequenceNumbersAtToolChange: false, // only show sequence numbers at tool change
  sequenceNumberStart: 10, // first sequence number
  sequenceNumberIncrement: 5, // increment for sequence numbers
  optionalStop: true, // optional stop
  separateWordsWithSpace: true, // specifies that the words should be separated with a white space
  useCycles: true // enable to output drilling cycles
};
...
*/
function writeBlock() {
  if (properties.showSequenceNumbers && !properties.showSequenceNumbersAtToolChange) {
...
function onSection() {
  ...
  if (false && (isFirstSection() || insertToolCall)) {
