...
properties = {
  ...
  useG53: true // disable to avoid G53 output for safe machine retracts - when disabled you must manually ensure safe retracts
};
...
function onSection() {
  ...
  if (insertToolCall ||
    ...
    if (properties.useActiveSpindle) {
      ...
      writeComment(
        "S" + rpmFormat.format(tool.spindleRPM) + " " + mFormat.format(tool.clockwise ? 3 : 4)
      );
      wait = true;
    }
  }
