...
description = "Generic HAAS UMC-750";
...
properties = {
  ...
  useMaximumMachineZRetract: false, // enable to use the maximum machine Z for retracts instead of G53 Z0
  forceHomeOnIndexing: true // force home position on indexing
};
...
function onOpen() {
  ...
  if (true) { // UMC-750
    ...
    if (properties.useMaximumMachineZRetract) {
  ...
  writeBlock(mFormat.format(31)); // Start Chip Conveyor
}
...
function onSection() {
  ...
  if (newWorkPlane) { // go to home position for safety
    if (!retracted) {
      retracted = true;
      if (properties.useG28) {
        writeBlock(gFormat.format(28), gAbsIncModal.format(91), "Z" + xyzFormat.format(0));
  ...
  if (insertToolCall || retracted ||
    ...
    if (!machineConfiguration.isHeadConfiguration()) {
      ...
      writeBlock(
        gMotionModal.format(0),
        conditional(!currentSection.isMultiAxis(), gFormat.format(43)),
