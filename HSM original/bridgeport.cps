/**
  ...
  FORKID {F3119F46-1AEA-47e8-94A1-41460DAD4328}
*/

description = "Generic Bridgeport";
vendor = "Autodesk, Inc.";
...
var gFeedModeModal = createModal({}, gFormat); // modal group 5 // G44-45
...
function writeBlock() {
  ...
  if (properties.showSequenceNumbers) {
    ...
    if (words) {
      writeWords2("N" + (sequenceNumber % 10000), words, ";");
...
function onOpen() {
  ...
  if (programName) {
    ...
    if (!((programId >= 1) && (programId <= 65536))) {
  ...
  writeBlock(gAbsIncModal.format(90), gFeedModeModal.format(44), gFormat.format(75), gPlaneModal.format(17));
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    if (properties.useToolChanger && properties.preloadTool) {
...
function onCyclePoint(x, y, z) {
  if (isFirstCyclePoint()) {
    ...
    // var P = (cycle.dwell == 0) ? 0 : clamp(1, cycle.dwell, 99999.999); // in seconds
    ...
    case "tapping":
      if (tool.type == TOOL_TAP_LEFT_HAND) {
        error(localize("Left handed tapping not supported."));
        return;
      }
      if (!F) {
