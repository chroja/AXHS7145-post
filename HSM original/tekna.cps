/**
  ...
  Tekna post processor configuration.

  $Revision: 40091 $
  $Date: 2015-10-14 17:29:32 +0200 (on, 14 okt 2015) $
  
  FORKID {69C75829-4F38-4a53-99D9-018631849B7A}
*/

description = "Generic Tekna";
vendor = "Autodesk, Inc.";
...
maximumCircularRadius = spatial(50, MM);
...
var abcFormat = createFormat({decimals:2, scale:DEG});
...
function writeComment(text) {
  writeln("/; " + text); // use // for non-displayed comments
}
...
function onSection() {
  ...
  if (insertToolCall) {
    ...
    gPlaneModal.reset();
    writeBlock(gPlaneModal.format(17), "T" + toolFormat.format(tool.number), "DL" + xyzFormat.format(tool.bodyLength));
    if (tool.comment) {
  ...
  if (insertToolCall ||
    ...
    if (tool.type == TOOL_TAP_LEFT_HAND) {
      writeBlock(
        sOutput.format(tool.spindleRPM), mFormat.format(4)
      );
    } else {
      writeBlock(
...
function onDwell(seconds) {
  ...
  // writeBlock("TR" + secFormat.format(seconds));
}
...
function getCommonCycle(x, y, z, r) {
  ...
  return [xOutput.format(x), yOutput.format(y),
    ...
    // "RA" + xyzFormat.format(r) // clearance
}
    ...
    case "chip-breaking":
      if (cycle.accumulatedDepth < cycle.depth) {
        ...
        writeBlock(
          ...
          getCommonCycle(x, y, z, cycle.retract),
          conditional(P > 0, "G4=" + secFormat.format(P)), // optional
          "D" + Math.max(Math.floor(cycle.depth/cycle.incrementalDepth), 1),
          "FP" + feedFormat.format(F)
        );
      }
      break;
    ...
    case "tapping":
      if (tool.type == TOOL_TAP_LEFT_HAND) {
        ...
        writeBlock(
          ...
          getCommonCycle(x, y, z, cycle.retract),
          "P" + xyzFormat.format(tool.threadPitch)
        );
      } else {
...
function onRapid(_x, _y, _z) {
  ...
  if (x || y || z) {
    ...
    if (true) {
      ...
      writeBlock(gMotionModal.format(0), x, y, z); // alternatively use G6
      feedOutput.reset();
    }
  }
}
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      writeBlock(gPlaneModal.format(17), gMotionModal.format(clockwise ? 2 : 3), iOutput.format(cx), jOutput.format(cy), feedOutput.format(feed));
      break;
    case PLANE_ZX:
      writeBlock(gPlaneModal.format(18), gMotionModal.format(clockwise ? 2 : 3), iOutput.format(cz), jOutput.format(cx), feedOutput.format(feed));
      break;
    case PLANE_YZ:
      writeBlock(gPlaneModal.format(19), gMotionModal.format(clockwise ? 2 : 3), iOutput.format(cy), jOutput.format(cz), feedOutput.format(feed));
      break;
    default:
      linearize(tolerance);
    }
    ...
    case PLANE_XY:
      ...
      break;
    case PLANE_ZX:
      writeBlock(gPlaneModal.format(18), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cz), jOutput.format(cx), feedOutput.format(feed));
      break;
    case PLANE_YZ:
      writeBlock(gPlaneModal.format(19), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cy), jOutput.format(cz), feedOutput.format(feed));
      break;
    default:
      linearize(tolerance);
    }
...
function onClose() {
  ...
  // writeBlock(gFormat.format(28), "Z" + xyzFormat.format(0)); // retract
  // zOutput.reset();
...
/*
  if (machineConfiguration.hasHomePositionX() || machineConfiguration.hasHomePositionY()) {
    ...
    writeBlock(gFormat.format(53), gMotionModal.format(0), homeX, homeY);
  }
*/
