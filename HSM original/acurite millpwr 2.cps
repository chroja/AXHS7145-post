/**
  ...
  Acu-Rite MILLPWR 2 post processor configuration.

  $Revision: 40091 $
  $Date: 2015-10-14 17:29:32 +0200 (on, 14 okt 2015) $
  
  FORKID {66EF661E-03D5-483f-B3E3-651343DA8AC8}
*/

description = "Acu-Rite MILLPWR 2";
vendor = "Acu-Rite";
...
var gPlaneModal = createModal({onchange:function () {gMotionModal.reset();}}, gFormat); // modal group 2 // G17
...
function onRapid(_x, _y, _z) {
  ...
  if (x && y && z) {
    error(localize("XYZ motion is not supproted."));
    return;
  }
  if (x || y) {
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    switch (getCircularPlane()) {
    case PLANE_XY:
      writeBlock(gPlaneModal.format(17), gMotionModal.format(clockwise ? 2 : 3), xOutput.format(x), yOutput.format(y), iOutput.format(cx - start.x, 0), jOutput.format(cy - start.y, 0), feedOutput.format(feed));
      break;
    default:
      linearize(tolerance);
    }
...
function onClose() {
  ...
  writeBlock(mFormat.format(2)); // end program
}
