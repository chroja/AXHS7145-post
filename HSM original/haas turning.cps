    ...
    case SPINDLE_PRIMARY: // main spindle
      if (true) {
        ...
        if (currentSection.getTool().getSpindleMode() == SPINDLE_CONSTANT_SURFACE_SPEED) {
          ...
          writeBlock(gSpindleModeModal.format(96), sOutput.format(tool.surfaceSpeed * ((unit == MM) ? 1/1000.0 : 1/12.0)), mFormat.format(tool.clockwise ? 3 : 4));
...
function onCyclePoint(x, y, z) {
  ...
  case "thread-turning":
    ...
    // writeBlock(mFormat.format(23)); // chamfer/fade on - fading is default behavior
    ...
    // writeBlock(mFormat.format(24)); // chamfer/fade off
    forceFeed();
    return;
  }
