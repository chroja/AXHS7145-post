/**
  ...
  $Revision: 40456 $
  $Date: 2015-12-15 07:56:03 +0100 (ti, 15 dec 2015) $
  
  FORKID {6B5CC843-2BF6-484f-A55D-74B1506FBCFF}
*/

description = "Generic 3-Axis Thermwood";
vendor = "Thermwood";
...
properties = {
  ...
  useCannedCycles: true, // output canned cycles instead of expanding them
  acceleration: 5,
  tangency: 1
};
...
function onOpen() {
  ...
  if (false) {
    ...
    var cAxis;
    if (properties.gotContinuousCAxis) {
      cAxis = createAxis({coordinate:2, table:false, axis:[0, 0, -1], cyclic:properties.gotContinuousCAxis, preference:0});
    } else {
  ...
  if ((properties.acceleration < 0) || (properties.acceleration >= 10)) {
    error(localize("Acceleration is out of range."));
    return;
  }
  writeBlock(gFormat.format(800 + properties.acceleration), formatComment("Acceleration Macro"));

  if ((properties.tangency <= 0) || (properties.acceleration > 40)) {
    error(localize("Tangency is out of range."));
    return;
  }
  writeBlock(gFormat.format(9), "F" + properties.tangency, formatComment("Tangency Factor"));
}
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    writeBlock(gFormat.format(990)); // reset / use machine coordinates
    zOutput.reset();
    writeBlock(gAbsIncModal.format(90), gMotionModal.format(0), zOutput.format(0)); // retract
  ...
  if (insertToolCall || forceToolCall ||
    ...
    writeBlock(
      sOutput.format(tool.spindleRPM), conditional(gotProgrammableSpindle, mFormat.format(tool.clockwise ? 3 : 4))
    );
  }
...
function onCyclePoint(x, y, z) {
  if (!properties.useCannedCycles || !currentSection.isZOriented()) {
...
function onClose() {
  ...
  if (!machineConfiguration.hasHomePositionX() && !machineConfiguration.hasHomePositionY()) {
    writeBlock(gAbsIncModal.format(90), gMotionModal.format(0), xOutput.format(0), yOutput.format(0)); // return to home
  } else {
    var homeX;
    ...
    writeBlock(gAbsIncModal.format(90), gMotionModal.format(0), homeX, homeY); // return to home
  }
