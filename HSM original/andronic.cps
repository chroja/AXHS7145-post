...
properties = {
  ...
  ausgeleichsfutter: false, // TAG: fix name for ausgeleichsfutter
  g288L: -1, // sets the G288 L address (-1 for disabled)
  g288H: -1, // sets the G288 H address (-1 for disabled)
  g288K: -1 // sets the G288 K address (-1 for disabled)
};
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    writeBlock(gMotionModal.format(0), "D0", "Z" + xyzFormat.format(0)); // retract
    writeBlock(gFormat.format(282), "A-1"); // part coordinate system
    zOutput.reset();
  }
  ...
  if (insertToolCall ||
    ...
    writeBlock(
      sOutput.format(tool.spindleRPM), (tool.clockwise ? mFormat.format(3) : mFormat.format(4))
    );
  }
  ...
  if ((properties.g288L != -1) || (properties.g288H != -1) || (properties.g288K != -1)) {
    writeBlock(
      gFormat.format(288),
      (properties.g288L ? ("L" + properties.g288L) : ""),
      (properties.g288H ? ("H" + properties.g288H) : ""),
      (properties.g288K ? ("K" + properties.g288K) : "")
    );
  }
...
function onCyclePoint(x, y, z) {
  if (isFirstCyclePoint()) {
    ...
    var P = (cycle.dwell == 0) ? 0 : clamp(0.001, cycle.dwell, 655); // in seconds
    ...
    case "tapping":
      if (tool.type == TOOL_TAP_LEFT_HAND) {
        ...
        writeBlock(
          ...
          getCommonCycle(cycle),
          "A" + (properties.ausgeleichsfutter ? 1 : 0),
