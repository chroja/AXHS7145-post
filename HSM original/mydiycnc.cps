/**
  ...
  MyDIYCNC - FABCAM post processor configuration.

  $Revision: 40469 $
  $Date: 2015-12-15 11:50:07 +0100 (ti, 15 dec 2015) $
  
  FORKID {865AACDB-474B-4750-BC63-ECD5AB7FA589}
*/

description = "Generic MyDIYCNC (FABCAM)";
vendor = "MyDIYCNC";
vendorUrl = "http://www.mydiycnc.com";
legal = "Copyright (C) 2012-2015 by Autodesk, Inc.";
certificationLevel = 2;
minimumRevision = 24000;

longDescription = "Generic MyDIYCNC for milling.";
...
properties = {
  accelerationTolerance: 0.01, // user defined tolerance for acceleration on
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    retracted = true;
    writeBlock(gFormat.format(31)); // retract
    zOutput.reset();
  }
  ...
  if (insertToolCall ||
    ...
    if (!tool.clockwise) {
      error(localize("Only clockwise spindle direction is supported by MyDIYCNC."));
      return;
    }
    writeBlock(
...
*/
  ...
  if (hasParameter("operation:tolerance")) {
    ...
    if (properties.accelerationTolerance <= tolerance) {
      accelerationEnabled = true;
      writeBlock(gFormat.format(60)); // switch acceleration ON
    } else {
      accelerationEnabled = false;
      writeBlock(gFormat.format(61)); // switch acceleration OFF
    }
  }
}
...
function onSpindleSpeed(spindleSpeed) {
  // not supported writeBlock(sOutput.format(spindleSpeed));
}
...
function onCyclePoint(x, y, z) {
  if (isFirstCyclePoint()) {
    ...
    case "deep-drilling":
      ...
      break;
    case "tapping":
      error(localize("Tapping is not supported by MyDIYCNC."));
      break;
    case "left-tapping":
      error(localize("Tapping is not supported by MyDIYCNC."));
      break;
...
function onLinear(_x, _y, _z, feed) {
  ...
  if (x || y || z) {
    if (pendingRadiusCompensation >= 0) {
      ...
      error(localize("Radius compensation Left/Right is not supported by MyDIYCNC."));
    } else {
...
var mapCommand = {
  ...
  // not supported COMMAND_END:2,
  COMMAND_SPINDLE_CLOCKWISE:3,
  // not supported COMMAND_SPINDLE_COUNTERCLOCKWISE:4,
  COMMAND_STOP_SPINDLE:5,
  // not supported COMMAND_ORIENTATE_SPINDLE:19,
  COMMAND_LOAD_TOOL:6
};
