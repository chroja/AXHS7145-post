...
function onCyclePoint(x, y, z) {
  if ((cycleType == "tapping") || (cycleType == "left-tapping") || (cycleType == "right-tapping")) {
    ...
    sOutput.reset();
    writeBlock("THREAD", sOutput.format(tool.spindleRPM), "L" + xyzFormat.format(-(cycle.retract - cycle.bottom)), "P" + pitchFormat.format(((tool.type == TOOL_TAP_LEFT_HAND) ? -1 : 1) * tool.threadPitch));
