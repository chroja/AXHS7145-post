...
function onSection() {
  ...
  if (insertToolCall ||
    ...
    if (isFirstSection()) { // TAG: if RPM changes
      var seconds = 3 * 60/tool.spindleRPM; // wait for 3 revolutions
  ...
  if (workOffset > 0) {
    if (properties.format == 1) {
      if (workOffset != currentWorkOffset) {
        writeBlock(eFormat.format(workOffset));
  ...
  if (insertToolCall || retracted) {
    ...
    if (x && y) {
      ...
      writeBlock(gAbsIncModal.format(90), gMotionModal.format(1), x, y, feedOutput.format(highFeedrate));
    } else {
...
function onCyclePoint(x, y, z) {
  if (isFirstCyclePoint()) {
    ...
    case "chip-breaking":
      ...
      if (dwell > 0) {
        ...
        writeBlock(
          ...
          "P" + xyzFormat.format(machineParameters.drillingSafeDistance),
          feedOutput.format(F)
        );
      }
      break;
    ...
    case "tapping":
      if (properties.format == 1) {
        writeBlock(
          gRetractModal.format(98), gAbsIncModal.format(90), gCycleModal.format((tool.type == TOOL_TAP_LEFT_HAND) ? 74.1 : 84.1),
          getCommonCycle(x, y, z, cycle.retract),
    ...
    case "left-tapping":
      if (properties.format == 1) {
        writeBlock(
          gRetractModal.format(98), gAbsIncModal.format(90), gCycleModal.format(74.1),
          getCommonCycle(x, y, z, cycle.retract),
          "Q" + xyzFormat.format(tool.threadPitch),
          "F" + feedFormat.format(tool.spindleRPM)
        );
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      writeBlock(gPlaneModal.format(17), gMotionModal.format(clockwise ? 2 : 3), crc, iOutput.format(cx - start.x, 0), jOutput.format(cy - start.y, 0), feedOutput.format(feed));
      break;
    case PLANE_ZX:
    ...
    switch (getCircularPlane()) {
    case PLANE_XY:
      writeBlock(gPlaneModal.format(17), gMotionModal.format(clockwise ? 2 : 3), crc, xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx - start.x, 0), jOutput.format(cy - start.y, 0), feedOutput.format(feed));
      break;
    case PLANE_ZX:
