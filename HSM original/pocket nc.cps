...
function onOpen() {
  ...
  if (true) { // only one kind of machine
    // var aAxis = createAxis({coordinate:0, table:true, axis:[-1, 0, 0], range:[-5.0001,95.0001], preference:1}); // use this later
    var aAxis = createAxis({coordinate:0, table:true, axis:[-1, 0, 0], range:[-180.0001,180.0001], preference:1}); // postpone some warnings about unsupported orientations until running the NC program
    var bAxis = createAxis({coordinate:1, table:true, axis:[0, -1, 0], range:[-9999,9999], preference:1});
...
/** Move to safe position before indexing rotaries. */
function moveToSafeIndexPosition() {
...
function onClose() {
  ...
  writeBlock(gFormat.format(49));
  moveToSafeIndexPosition(); // always go home
