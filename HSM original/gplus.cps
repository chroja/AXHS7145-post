...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    retracted = true;
    writeBlock(gFormat.format(53), gAbsIncModal.format(91), gMotionModal.format(0), "Z" + xyzFormat.format(0)); // retract
...
function onCyclePoint(x, y, z) {
  if (isFirstCyclePoint()) {
    ...
    var P = (cycle.dwell == 0) ? 0 : clamp(1, 1000 * cycle.dwell, 99999); // in milliseconds
...
function onClose() {
  ...
  if (!machineConfiguration.hasHomePositionX() && !machineConfiguration.hasHomePositionY()) {
    writeBlock(gFormat.format(53), gAbsIncModal.format(91), gMotionModal.format(0), "X" + xyzFormat.format(0), "Y" + xyzFormat.format(0)); // return to home
  } else {
    var homeX;
