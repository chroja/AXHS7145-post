/**
  ...
  Milltronics post processor configuration.

  $Revision: 40469 $
  $Date: 2015-12-15 11:50:07 +0100 (ti, 15 dec 2015) $
  
  FORKID {E067555C-4636-446b-940F-5787D6F23DC0}
*/

description = "Generic Milltronics";
vendor = "Autodesk, Inc.";
...
properties = {
  ...
  G18: "" // set to "ZX" (right handed) or "XZ" (left handed) in accordance with your control setting - byte 79 - G18 plane select: XZ=0, ZX=1
};
...
var secFormat = createFormat({decimals:2}); // seconds - range 0.01-99999.99
...
function onParameter(name, value) {
  if (name == "display") {
    writeBlock("PRINT" + "[" + filterText(String(value).toUpperCase(), permittedCommentChars) + "]");
  }
}
...
function onSection() {
  ...
  if (insertToolCall || newWorkOffset || newWorkPlane) {
    ...
    writeBlock(gFormat.format(32)); // retract
    zOutput.reset();
  }
  ...
  if (workOffset > 0) {
    if (workOffset > 6) {
      // G540->G599 - but G54=G540, G55=G550, ...
...
function onCyclePoint(x, y, z) {
  if (isFirstCyclePoint()) {
    ...
    var P = (cycle.dwell == 0) ? 0 : clamp(0.01, cycle.dwell, 99999.999); // in seconds
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    case PLANE_XY:
      ...
      break;
    case PLANE_ZX:
      if (properties.G18 == "ZX") { // right handed
      ...
      } else if (properties.G18 == "XZ") { // left handed
        writeBlock(gAbsIncModal.format(90), gPlaneModal.format(18), gMotionModal.format(clockwise ? 3 : 2), iOutput.format(cx - start.x, 0), kOutput.format(cz - start.z, 0), feedOutput.format(feed));
      } else {
        warningOnce("ATTENTION: Set the G18 property to either 'ZX' (right handed) or 'XZ' left handed.", WARNING_G18);
        warningOnce("ATTENTION: For more information see programming manual - about byte 79 - G18 plane select: XZ=0, ZX=1.", WARNING_G18_2);
        linearize(tolerance);
      }
      break;
    case PLANE_YZ:
    ...
    case PLANE_ZX:
      if (properties.G18 == "ZX") { // right handed
        ...
        writeBlock(gAbsIncModal.format(90), gPlaneModal.format(18), gMotionModal.format(clockwise ? 3 : 2), xOutput.format(x), yOutput.format(y), zOutput.format(z), iOutput.format(cx - start.x, 0), kOutput.format(cz - start.z, 0), feedOutput.format(feed));
      } else {
        warningOnce("ATTENTION: Set the G18 property to either 'ZX' (right handed) or 'XZ' left handed.", WARNING_G18);
        warningOnce("ATTENTION: For more information see programming manual - about byte 79 - G18 plane select: XZ=0, ZX=1.", WARNING_G18_2);
        linearize(tolerance);
    ...
    case PLANE_ZX:
      if (properties.G18 == "ZX") { // right handed
        ...
        writeBlock(gPlaneModal.format(18), gMotionModal.format(clockwise ? 3 : 2), xOutput.format(x), yOutput.format(y), zOutput.format(z), "R" + rFormat.format(r), feedOutput.format(feed));
      } else {
        warningOnce("ATTENTION: Set the G18 property to either 'ZX' (right handed) or 'XZ' left handed.", WARNING_G18);
        warningOnce("ATTENTION: For more information see programming manual - about byte 79 - G18 plane select: XZ=0, ZX=1.", WARNING_G18_2);
        linearize(tolerance);
