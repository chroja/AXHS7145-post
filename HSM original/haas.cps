...
longDescription = "Generic post for use with all common 3-axis HAAS mills like the DM, VF, EC, Office Mill, and Mini Mill series.";
...
function onOpen() {
  ...
  if (properties.optionallyMeasureTools) {
    var tools = getToolTable();
    if (tools.getNumberOfTools() > 0) {
      writeln("");
...
function onSection() {
  ...
  if (properties.useParametricFeed &&
      ...
      !(useInverseTimeFeed && currentSection.isMultiAxis())) {
    if (!insertToolCall &&
        activeMovements &&
...
function onSectionEnd() {
  ...
  if (currentSection.isMultiAxis()) {
    if (useInverseTimeFeed) {
      writeBlock(gFormat.format(94));  // inverse time feed off
    }
  }
}
